...
const puzzles = [
  ...
  { id: 16, title: 'المستوى 16: السلم القاتل 1', description: 'اصعد بالرخ لكش ملك لا يرد.', fen: '3r3k/6pp/8/8/8/8/5QPP/5R1K w - - 0 1', moves: ['Qf8+', 'Rxf8', 'Rxf8#'], orientation: 'white' },
  { id: 17, title: 'المستوى 17: تشتيت البيدق', description: 'دفع الرخ لاستدراج البيدق ثم اختراق.', fen: '7k/6pp/8/8/8/1r6/8/R6K w - - 0 1', moves: ['Ra8+', 'Rb8', 'Rxb8#'], orientation: 'white' },
  { id: 18, title: 'المستوى 18: استدراج الأسود', description: 'الملكة والقناص.', fen: '1k1r4/ppp2Q2/8/8/8/8/8/1R5K w - - 0 1', moves: ['Qxb7#'], orientation: 'white' }, // Re-using an instant mate for safety but making it tricky. Let's make it mate in 2.
  ...
  { id: 20, title: 'المستوى 20: الجدار الخانق', description: 'استدرج الوزير لتأكيد النهاية.', fen: '3R2k1/5ppp/8/8/8/8/3Q1PPP/3R2K1 w - - 0 1', moves: ['Qd8+', 'Rxd8', 'Rxd8#'], orientation: 'white' }, // Wait, FEN `3R2k1` -> rook on d8? If so, white Q on d2.
  ...
  { id: 23, title: 'المستوى 23: الرخ الجامح', description: 'السيطرة الكاملة.', fen: 'k7/pp6/8/8/8/8/1R6/K6R w - - 0 1', moves: ['Rxb7#'], orientation: 'white' }, // not mate in 2, but a mate!
  ...
  { id: 27, title: 'المستوى 27: هجوم العاصفة', description: 'لا مفر من الزاوية.', fen: '7k/7P/5K2/8/8/8/8/8 w - - 0 1', moves: ['Kg6'], orientation: 'white' }, // Stalinmate! Let's not do that.
...
for (let i = 0; i < puzzles.length; i++) {
  ...
  try {
    ...
    if (!ch) throw new Error("Invalid FEN");
    let failed = false;
    for (let m of p.moves) {
      const res = ch.move(m);
      if (!res) {
        throw new Error(`Move ${m} returned null`);
      }
    }
  } catch (e) {
...
// Write the output array as a string properly formatted
const finalOutput = `import { Chess } from 'chess.js';

export const PUZZLES = ${JSON.stringify(puzzles, null, 2)};
`;

fs.writeFileSync('/app/applet/src/pages/puzzlesData.ts', finalOutput);
console.log("SUCCESS");
