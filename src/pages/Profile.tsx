...
export const Profile: React.FC = () => {
  ...
  const wins = history.filter(
    (g) =>
      g.userResult === "win" || (!g.userResult && g.outcome.includes("فاز")),
  ).length;
  const losses = history.filter(
    (g) =>
      g.userResult === "loss" ||
      (!g.userResult &&
        !g.outcome.includes("فاز") &&
        !g.outcome.includes("تعادل") &&
        g.outcome.includes("!")),
  ).length;
  const draws = history.filter(
    (g) =>
      ...
      (!g.userResult &&
        (g.outcome.includes("تعادل") ||
          g.outcome.includes("تكرار") ||
          g.outcome.includes("ستالميت"))),
  ).length;
          ...
          <div className="lg:col-span-1 flex flex-col gap-4">
            <div className="bg-[#161616] border border-[#2a2a2a] p-6 rounded-xl shadow-xl flex flex-col items-center text-center">
              <div className="relative group cursor-pointer w-28 h-28 mb-4">
                ...
                <div className="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  <span className="text-white text-xs font-bold px-2 text-center text-balance leading-tight">
                    تغيير الصورة
                  </span>
                </div>
              </div>
              ...
              <p className="text-amber-500 text-lg font-mono tracking-widest uppercase">
                التقييم: {rating}
              </p>

              <div className="w-full border-t border-[#2a2a2a] mt-6 pt-6 grid grid-cols-3 gap-2">
                <div className="flex flex-col items-center">
                  <span className="text-green-500 text-xs uppercase mb-1">
                    فوز
                  </span>
            ...
            {history.length > 0 && (
              <button
                ...
                className="w-full py-3 rounded-lg border border-red-900/50 bg-red-900/10 hover:bg-red-900/20 text-red-500 font-bold transition-colors"
              >
                مسح السجل
              </button>
            )}
              ...
              <div className="space-y-4">
                {history
                  .map((record, idx) => (
                    <div
                      key={`${record.id}-${idx}`}
                      className="bg-[#111] border border-[#2a2a2a] p-4 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4 hover:border-amber-500/50 transition-colors"
                    >
                      <div className="flex items-center gap-4 w-full sm:w-auto">
                        <div
                          className={clsx(
                            "w-12 h-12 rounded-full flex items-center justify-center text-white shrink-0 shadow-inner",
                            record.outcome.includes("فاز")
                        ...
                        <div>
                          ...
                          <div className="text-gray-500 text-[10px] mt-1 font-mono tracking-widest">
                      ...
                      >
                        <Play size={16} className="text-amber-500" /> إعاده
                        العرض
                      </button>
                    </div>
                  ))
        ...
        <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-8 bg-[#161616] border border-[#2a2a2a] p-6 rounded-xl animate-fade-in">
          <div className="w-full lg:w-[500px] flex-shrink-0">
            <div className="flex justify-between items-center mb-4">
              <button
                onClick={closeReplay}
                className="text-gray-400 hover:text-white flex items-center gap-2 text-sm bg-[#222] px-3 py-1.5 rounded border border-[#2a2a2a]"
              >
                <X size={16} /> إغلاق
              </button>
            ...
            <div className="flex justify-between items-center mt-6 bg-[#111] p-2 rounded-lg border border-[#2a2a2a]">
              <button
                ...
                className="p-3 text-white disabled:opacity-30 hover:bg-[#222] rounded transition-colors bg-[#222]"
              >
                <ArrowRight size={20} />
              </button>
              ...
              <button
                onClick={stepForward}
                disabled={replayIndex === replayingGame.moves.length}
                className="p-3 text-white disabled:opacity-30 hover:bg-[#222] rounded transition-colors bg-[#222]"
              >
                <ArrowLeft size={20} />
              </button>
          ...
          <div className="flex-1 w-full flex flex-col gap-4">
            ...
            <div className="bg-[#111] border border-[#2a2a2a] p-6 rounded-xl text-center">
              <h3 className="text-2xl font-bold text-white mb-2">
                {replayingGame.outcome}
              </h3>
              <p className="text-gray-400">
                الخصم:{" "}
                <strong className="text-amber-500">
                  {replayingGame.opponent}
                </strong>
              </p>
              <p className="text-gray-600 text-xs mt-2 font-mono">
                {new Date(replayingGame.date).toLocaleString("ar-EG")}
              </p>
            </div>

            {/* Moves */}
            <div className="bg-[#111] border border-[#2a2a2a] p-4 rounded-xl flex-1 max-h-[350px] flex flex-col">
              <h4 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-3 border-b border-[#2a2a2a] pb-2">
                سجل الحركات
              </h4>
              <div
                className="flex-1 overflow-y-auto custom-scrollbar font-mono text-sm pr-2"
                dir="ltr"
              >
                {Array.from({
                  length: Math.ceil(replayingGame.moves.length / 2),
                }).map((_, i) => (
                  <div
                    key={i}
                    className={clsx(
                      "flex justify-between px-3 py-1.5 rounded transition-colors group",
                      replayIndex > i * 2
